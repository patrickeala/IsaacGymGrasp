...
class IsaacGymSim:
    def __init__(self, args):
        ...
        # ===== Creating obj asset =====
        self.obj_asset_file = self.load_as_urdf(obj_name="mug", asset_dir=self.obj_asset_root, obj_path=self.obj_path, texture_obj_path=self.obj_path, scale=1)
        self.obj_asset = self.create_obj_asset(self.sim, self.obj_asset_root, self.obj_asset_file, self.obj_asset_options())

        # ===== Creating gripper asset =====
        self.gripper_asset = self.create_gripper_asset()
    ...
    def create_envs(self, num_envs=256):
        ...
        # ===== Object pose  =====
        self.obj_pose = self.get_object_pose()

        for i in range(num_envs):
            env = self.gym.create_env(self.sim, self.env_lower, self.env_upper, self.num_per_row)
            self.envs.append(env)

            self.create_obj_actor(env, self.obj_asset, self.obj_pose, "mug", i, 0)
