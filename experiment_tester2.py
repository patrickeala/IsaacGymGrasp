...
if __name__ == "__main__":
    ...
    data = pickle.load(open('experiment1_test.pkl', 'rb'))
    ...
    isaac_labels = utils.simulate_isaac(translations, quaternions, obj_pose_relative, scale=1,
                path_to_obj_mesh=path_to_obj_mesh, headless=headless)
    isaac_labels = isaac_labels.astype(int)
    print("index of successful grapsp: ",isaac_labels )
    data.append(isaac_labels)
    with open("experiment1_test_labelled.pkl","wb") as handle:
