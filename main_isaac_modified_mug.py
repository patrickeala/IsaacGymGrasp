import IsaacGymSimulator_modified_shapenet_mug as isaac_sim
...
if __name__ == "__main__":
    ...
    category= "mug"
    data_dir = f"grasps/{category}/grasps/"
    ...
    for sample in os.listdir(f'{data_dir}'):
        ...
        object_name = category + filename.split(category)[1][:3]
        print("grasp_file: ", grasp_file)
        print("filename: ", filename)
        print("object name: ", object_name)        
        isaac = isaac_sim.IsaacGymSim(args)
        print(f"getting labels for {filename}")
        isaac.set_paths(cat=category,obj=object_name,grasps_file=grasp_file,is_shapenet=True)
        isaac.process_grasps(index_of_grasps=np.random.randint(1430000, size=(1, 10000)))
        ...
        isaac.print_results()
        print("----------------------")
        print(f"done with {object_name}")
        print("----------------------")
        # isaac.step_simulation(800)
        isaac.cleanup()


        del isaac, env
        gc.collect()
        torch.cuda.empty_cache()    # isaac.execute_grasp()
        # isaac.check_grasp_success_pos()
        # isaac.save_isaac_labels(filename)        
        


        #### mug with issue 10 3 7 19 8 6 14 17 2  20